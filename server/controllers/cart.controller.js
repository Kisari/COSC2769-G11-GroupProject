...
module.exports.get = async (req, res) => {
  ...
  try {
    let allItems = await CartDetails.find({ cartId }).populate(
      "productId",
      "name"
    );
...
module.exports.add = async (req, res) => {
  ...
  try {
    ...
    if (cart) {
      let cartDetails = await CartDetails.findOne({
        cartId: cart._id,
        productId,
      });
      ...
      if (cartDetails) {
        ...
        var newItem = await CartDetails.create({
          cartId: cart._id,
          productId,
          quantity,
        });
        cart.numberOfItems += 1;
        res.status(200).json({ cart, newItem });
        await cart.save();
      }
    } else {
      let newCart = await Cart.create({ customerId, numberOfItems: 1 });
      let newCartDetails = await CartDetails.create({
        cartId: newCart._id,
        productId,
        quantity,
      });

      res.status(200).json({ newCart, newCartDetails });
    }
  } catch (err) {
